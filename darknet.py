...
def get_test_input():
    ...
    #img是【h,w,channel】，这里的img[:,:,::-1]是将第三个维度channel从opencv的BGR转化为
    #pytorch的RGB，然后transpose((2,0,1))的意思是将
...
def parse_cfg(cfgfile):
    ...
    """
    # 加载文件并过滤掉文本中多余内容
    file = open(cfgfile, 'r',encoding='UTF-8')
    ...
    for line in lines:
        if line[0] == "[":               # 这是cfg文件中一个层(块)的开始           
            ...
            #左边是key(去掉右空格)，右边是value(去掉左空格)，形成一个block字典的键值对
...
#PyTorch 为 convolutional 和 upsample 提供预置层。
...
def create_modules(blocks):
    ...
    output_filters = []
    #我们不仅需要追踪前一层的卷积核数量，还需要追踪之前每个层。
    #随着不断地迭代，我们将每个模块的输出卷积核数量添加到 output_filters 列表上。
    '''
    在迭代该列表之前，我们先定义变量 net_info，来存储该网络的信息。 
    我们的函数将会返回一个 nn.ModuleList。
    这个类几乎等同于一个包含 nn.Module 对象的普通列表。
    然而，当添加 nn.ModuleList 作为 nn.Module 对象的一个成员时（即当我们添加模块到我们的网络时），
    所有 nn.ModuleList 内部的 nn.Module 对象（模块）的 parameter 也被添加作为 nn.Module 对
    象（即我们的网络，添加 nn.ModuleList 作为其成员）的 parameter。 
    当我们定义一个新的卷积层时，我们必须定义它的卷积核维度。
    虽然卷积核的高度和宽度由 cfg 文件提供，但卷积核的深度是由上一层的卷积核数量（或特征图深度）决定的。
    这意味着我们需要持续追踪被应用卷积层的卷积核数量。
    我们使用变量 prev_filter 来做这件事。我们将其初始化为 3，因为图像有对应 RGB 通道的 3 个通道。 
    ...
    for index, x in enumerate(blocks[1:]):
        ...
        if (x["type"] == "convolutional"):
            ...
            #string{a}.format(B)   用B替换{}位置，a表示多个替换时的次序   
